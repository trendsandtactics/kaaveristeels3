...
export default async function ModuleDetailPage({ params }: { params: Promise<{ module: string; slug: string }> }) {
  ...
  const image = item.cover_image ? resolveMediaUrl(item.cover_image, "") : "";
  ...
  return (
    <main className="min-h-screen pt-24 bg-gray-50">
      ...
      <section className="max-w-5xl mx-auto px-6 py-10">
        {image ? <div className="relative w-full rounded-2xl border border-black/10 h-[360px] overflow-hidden"><Image src={image} alt={title} fill className="object-cover" sizes="100vw" /></div> : <div className="w-full rounded-2xl border border-black/10 h-[360px] bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center text-black/50">No image uploaded for this entry</div>}
