...
export default function Header() {
  ...
  const headerClass = clsx(
    ...
    transparent ? "bg-transparent py-6" : "bg-white shadow-sm py-4 border-b border-gray-200",
  );

  const navLinkClass = clsx(
    "font-body text-[10px] uppercase tracking-[0.18em] font-semibold pb-1 border-b-2 border-transparent transition-colors",
    transparent ? "text-white/90 hover:text-white hover:border-white" : "text-black hover:text-accent-red hover:border-accent-red",
  );

  const menuTriggerClass = clsx(
    "font-body text-[10px] uppercase tracking-[0.18em] font-semibold transition-colors",
  ...
  return (
    <header className={headerClass}>
      <div className="max-w-7xl mx-auto px-6 md:px-12 flex items-center justify-between">
        ...
        <nav className="hidden lg:flex items-center gap-5">
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            <div
              role="menu"
              className={clsx(
                "absolute right-0 top-full mt-1 w-[360px] max-h-[65vh] overflow-y-auto scroll-smooth bg-white border border-gray-200 shadow-[0_18px_40px_rgba(0,0,0,0.14)] p-4 rounded-md transition-all duration-200",
              ...
              <div className="flex flex-col">
                {mediaSupportLinks.map((page, index) => (
                  <Link
                    ...
                    className={clsx(
                      "px-2 py-2 text-[15px] text-black hover:text-accent-red transition-colors",
        ...
        <button
          className={clsx("lg:hidden z-50 w-8 h-8 flex flex-col justify-center items-end gap-1", transparent ? "text-white" : "text-black")}
          onClick={() => setMobileMenuOpen((prev) => !prev)}
          aria-label="Toggle menu"
        >
          <span className={clsx("block h-[2px]", transparent ? "bg-white" : "bg-black", mobileMenuOpen ? "w-5 rotate-45 translate-y-[6px]" : "w-6")} />
          <span className={clsx("block h-[2px]", transparent ? "bg-white" : "bg-black", mobileMenuOpen ? "opacity-0 w-5" : "w-5")} />
          <span className={clsx("block h-[2px]", transparent ? "bg-white" : "bg-black", mobileMenuOpen ? "w-5 -rotate-45 -translate-y-[6px]" : "w-3")} />
        </button>
          ...
          {mobileMenuOpen && (
            ...
            >
              <div className="w-full max-w-md space-y-6 max-h-[78vh] overflow-y-auto px-2">
                {[
                  { title: "Main Pages", links: [...navLinks, { name: "Careers", href: "/careers" }] },
                  { title: "Media & Support", links: mediaSupportLinks },
                ].map((category) => (
