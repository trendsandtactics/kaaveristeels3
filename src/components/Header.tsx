...
export default function Header() {
    ...
    const headerBg = isHeroSection ? "bg-transparent py-6" : "bg-white shadow-sm py-4 border-b border-gray-200";
    ...
    return (
        ...
        >
            <div className="max-w-7xl mx-auto px-6 md:px-12 flex items-center justify-between">
                ...
                <nav className="hidden lg:flex items-center gap-5">
                    {navLinks.map((link) => (
                        <Link
                            key={link.name}
                            href={link.href}
                            className={`relative font-body text-[10px] md:text-xs uppercase tracking-[0.2em] transition-colors group overflow-hidden font-semibold ${isHeroSection ? "text-white/90 hover:text-white" : "text-black hover:text-accent-red"}`}
                        >
                            {link.name}
                            <span className={`absolute bottom-0 left-0 w-full h-[2px] transform -translate-x-full transition-transform duration-300 group-hover:translate-x-0 ${isHeroSection ? "bg-white" : "bg-accent-red"}`} />
                        </Link>
                    ))}
                ...
                <button
                    className={`lg:hidden z-50 w-8 h-8 flex flex-col justify-center items-end gap-1 focus:outline-none ${isHeroSection ? "text-white" : "text-black"}`}
                    onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
                >
                    <span className={`block h-[2px] transition-all duration-300 ${isHeroSection ? "bg-white" : "bg-black"} ${mobileMenuOpen ? 'w-5 rotate-45 translate-y-[6px]' : 'w-6'}`} />
                    <span className={`block h-[2px] transition-all duration-300 ${isHeroSection ? "bg-white" : "bg-black"} ${mobileMenuOpen ? 'w-5 opacity-0' : 'w-5'}`} />
                    <span className={`block h-[2px] transition-all duration-300 ${isHeroSection ? "bg-white" : "bg-black"} ${mobileMenuOpen ? 'w-5 -rotate-45 -translate-y-[6px]' : 'w-3'}`} />
                </button>
