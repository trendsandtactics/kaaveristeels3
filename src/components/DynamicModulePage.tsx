            ...
            <div className="grid md:grid-cols-3 gap-5">
              {featured.map((item) => (
                <article key={item.id} className="rounded-2xl border border-black/10 bg-white overflow-hidden shadow-sm hover:shadow-xl transition-shadow">
                  {item.cover_image ? (
                    <div className="relative h-44 w-full"><Image src={resolveMediaUrl(item.cover_image, "") || "/image/kaaveriwbg.png"} alt={item.title} fill className="object-cover" sizes="(max-width: 768px) 100vw, 33vw" /></div>
                  ) : (
                    <div className="h-44 w-full bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center text-xs uppercase tracking-[0.18em] text-black/45">No Image</div>
                  )}
                  <div className="p-5">
          ...
          {items.map((item) => (
            <article key={item.id} className="group rounded-2xl border border-black/10 bg-white overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-300 hover:-translate-y-1">
              <div className="relative h-56 w-full">
                {item.cover_image ? (
                  <Image src={resolveMediaUrl(item.cover_image, "") || "/image/kaaveriwbg.png"} alt={item.title} fill className="object-cover" sizes="(max-width: 768px) 100vw, (max-width: 1280px) 50vw, 33vw" />
                ) : (
